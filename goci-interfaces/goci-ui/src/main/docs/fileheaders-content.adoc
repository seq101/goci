...
MAPPED GENE(S)^*^: Gene(s) mapped to the strongest SNP. If the SNP is located within a gene, that gene is listed. If the VARIANT is intergenic, the upstream and downstream genes are listed, separated by a hyphen.
...
STRONGEST SNP-EFFECT ALLELE^*^: VARIANT(s) most strongly associated with trait + effect allele (? for unknown effect allele). May also refer to a haplotype.

SNPS^*^: Strongest SNP; if a haplotype it may include more than one rs number (multiple SNPs comprising the haplotype)

MERGED^*^: denotes whether the VARIANT has been merged into a subsequent rs record (0 = no; 1 = yes;)
...
PLATFORM (SNPS PASSING QC)^*^: Genotyping platform manufacturer used in Stage 1; also includes notation of pooled DNA study design or imputation of VARIANTs, where applicable
